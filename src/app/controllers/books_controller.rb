...
class BooksController < ApplicationController
  include BooksHelper
  before_action :authenticate_user!, only: %i[show index edit]
  ...
  def update
    ...
    if @update_for_book.valid?
      ...
      if params[:book][:image].class == ActionDispatch::Http::UploadedFile
        @book.update_attributes(image: params[:book][:image])
      end
